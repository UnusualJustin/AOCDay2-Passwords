...
namespace AOCDay2_Passwords
{
    class Program
    {
        ...
        static bool TestPasswordMinMax(int minOccurrences, int maxOccurrences, char character, string password)
        {
            int occurrences = password.Where(c => c == character).Count();
            return occurrences >= minOccurrences && occurrences <= maxOccurrences;
        }

        static bool TestPasswordPositionalOccurance(int position1, int position2, char character, string password)
        {
            char characterAtPostion1 = password[position1 - 1];
            char characterAtPostion2 = password[position2 - 1];
